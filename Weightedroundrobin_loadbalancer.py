Weighted Round Robin Customized Controller:
"""
Customized Controller for performing Load balancing: CSE5300 ACN project
Load balancing technique: Weighted Round Robin
...
class iplb (EventMixin):
  ...
  def Weighted_RR_load_servers (self):
    # Weighted RR: load the servers
      global wet
      total=0
      for w in wet:
           total = total + int(w)
  ...
  def handle_request (self, packet, event):
    ...
    server = self. Weighted_RR_load_servers ()
...
def launch (ip,servers,wt):
  global IP
  global servers1
global wet
  servers1 = servers.replace(","," ").split()
  wet = wt.replace(","," ").split()
