Round Robin Customized Controller:
"""
...
class iplb (EventMixin):
  ...
  def Round_Robin_load_servers (self):
    # Round-robin load the servers
    self.server = (self.server + 1) % len(self.serverslist)
    return self.serverslist[self.server]
  ...
  def handle_request (self, packet, event):
    ...
    server = self. Round_Robin_load_servers ()
